...
Observação: As chaves primárias artificiais são geradas automaticamente pelo sistema de banco de dados no momento em documento é feito. Estes documentos recebem uma chave com o nome ``{\under}id'' onde esta já vem as regras de unicidade, e uma hash é gerada pela função \verb+ObjectId()+, estas foram omitidas de declaração no Diagrama de Entidades.
